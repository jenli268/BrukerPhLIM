...
FILE_PATH="D:\\DATA\\Data20220816\\"
FILE_NAME="LifetimeData_PLIM_Cycle00001_000001.sdt"

FILE_READ=FILE_PATH + FILE_NAME
...
def writeSdtToTxt(t, dataSDT):
    [rows, cols] = dataSDT.shape

    if len(t) != cols:
        raise Exception("Length of t: {} and dataSDT columns: {} is not equal".format(len(t), cols))

    output_file_name = "Python_Raw_Decay.asc"
    output_file = FILE_PATH + output_file_name

    print(output_file)
    with open(output_file, 'w') as f:
        for col in range(cols):
            output_y = [str(dataSDT[row][col]) for row in range(rows)]
            output_line = str(t[col] * 1e6) + " " + " ".join(output_y)
            # print(output_line)
            f.write(output_line)
            f.write("\n")
...
if __name__ == '__main__':
    (t, dataSDT) = read_sdt150(FILE_READ)
    writeSdtToTxt(t, dataSDT)
